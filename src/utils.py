...
def gray_to_bgr(gray_image):
    ...
    getColorMap = pyplot.get_cmap('jet')
...
def overlay_image_center(image_above, image_below):
    # image_above, image_below: numpy.ndarray from cv2.imread
    # return result of image_above overlays on the center of image_below
    if image_above.shape[0] > image_below.shape[0] or image_above.shape[1] > image_below.shape[1]:
        raise Exception('invalid image size for overlay')

    height_above = image_above.shape[0]
    width_above = image_above.shape[1]
    height_below = image_below.shape[0]
    width_below = image_below.shape[1]

    y = math.floor((height_below - height_above) / 2)
    x = math.floor((width_below - width_above) / 2)

    image_above_type = check_image(image_above)
    image_below_type = check_image(image_below)
    if image_below_type == 'grayscale':
        image_below = cv2.cvtColor(image_below, cv2.COLOR_GRAY2BGR)
    elif image_below_type == 'color':
        pass
    elif image_below_type == 'transparent':
        image_below = cv2.cvtColor(image_below, cv2.COLOR_BGRA2BGR)
    else:
        raise Exception('invalid image_below type')

    if image_above_type == 'grayscale':
        image_above = cv2.cvtColor(image_above, cv2.COLOR_GRAY2BGR)
        for c in range(0, 3):
            image_below[y:y + height_above, x:x + width_above, c] = image_above[:, :, c]
    elif image_above_type == 'color':
        for c in range(0, 3):
            image_below[y:y + height_above, x:x + width_above, c] = image_above[:, :, c]
    elif image_above_type == 'transparent':
        alpha_above = image_above[:, :, 3] / 255.0
        alpha_below = 1.0 - alpha_above
        for c in range(0, 3):
            image_below[y:y + height_above, x:x + width_above, c] = alpha_above * image_above[:, :, c] + alpha_below * image_below[y:y + height_above, x:x + width_above, c]
    else:
        raise Exception('invalid image_above type')

    # cv2.imshow('image_below', image_below)
    # cv2.waitKey()

    return image_below


def dilate_mask_tensor(image, kernel_size):
    # image: tensor
    # kernel_size: int>1
    padding_0 = (kernel_size - 1) // 2
    if kernel_size % 2 == 0:
        padding_1 = padding_0 + 1
    else:
        padding_1 = padding_0
    image_pad = functional.pad(image, pad=(padding_0, padding_1, padding_0, padding_1))

    kernel = torch.ones((1, 1, kernel_size, kernel_size))  # size: (1, 1, kernel_size, kernel_size)
    if image.is_cuda:
        kernel = kernel.cuda()
    dilated_image = torch.clamp(functional.conv2d(image_pad, kernel), 0, 1)
