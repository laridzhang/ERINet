...
from src.utils import log, is_only_one_bool_is_true, ExcelLog, compare_result, gray_to_bgr, make_path
...
if __name__ == '__main__':
    ...
    # initialize net
    net = CrowdCount(bce_init_weights=None)
    ...
    standard_error_dict['correct_cent'] = 0.0
    ...
    for epoch in range(max_epoch):
        ...
        for blob in data_tqdm:
            ...
            estimate_label, loss_dict, visual_dict = net(image, ground_truth=ground_truth_label)
            ...
            #     estimate_flag = torch.argmax(estimate_mask, dim=1, keepdim=True).cpu()
            #     ground_truth_flag = torch.argmax(ground_truth_mask, dim=1, keepdim=True)
            ...
            #     print('epoch: %4d, step %6d, correct cent: %f' % (epoch, step, correct_cent), flush=True)

            str_GTL = ' '.join('%.1f' % i for i in ground_truth_label[0])
            str_EL = ' '.join('%.1f' % i for i in estimate_label[0])
            data_tqdm.set_description(desc='epoch: %4d, ground truth: %s, estimate: %s and so on' % (epoch, str_GTL, str_EL), refresh=False)
