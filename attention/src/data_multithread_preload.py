...
class PreloadData:
    def __init__(self, icon_path, background_path, is_random_background=True, is_preload=False, is_transfrom=False, is_transform_in_gray=False):
        ...
        self.pickle_path = os.path.join(r'C:\pickle', '%4d%02d%02d%02d%02d%02d%06d_%s' %
        ...
        for filename in self.icon_filename_list:
            if self.is_preload:
                if psutil.virtual_memory().available > self.min_available_memory:
                    index += 1
                    self.preload_icon_dict[filename] = self.read_icon_blob(filename)
                    if index % 100 == 0:
    ...
    def read_icon_blob(self, filename):
        image = cv2.imread(join(self.icon_path, filename), cv2.IMREAD_UNCHANGED)
    ...
    def get_icon_by_index(self, index):
        ...
        icon_blob = self.preload_icon_dict[icon_filename]

        if icon_blob is None:  # no data is preloaded
            pickle_file_path = os.path.join(self.pickle_path, icon_filename + '.pickle')
            if os.path.isfile(pickle_file_path):
                with open(pickle_file_path, 'rb') as f:
                    icon_blob = pickle.load(f)
            else:
                icon_blob = self.read_icon_blob(icon_filename)
                with open(pickle_file_path, 'wb') as f:
                    pickle.dump(icon_blob, f)

        return icon_blob, icon_name
    ...
    def pad_mask(self, mask, height, width):
        # pad mask to fit expected height and width
        mask_height = mask.shape[0]
        mask_width = mask.shape[1]
        if mask_height < height:
            mask = cv2.copyMakeBorder(mask, 0, height - mask_height, 0, 0, cv2.BORDER_CONSTANT, value=0)
        elif mask_height == height:
            pass
        else:
            raise Exception('mask_height %d is larger than expected %d' % (mask_height, height))
        if mask_width < width:
            mask = cv2.copyMakeBorder(mask, 0, 0, 0, width - mask_width, cv2.BORDER_CONSTANT, value=0)
        elif mask_width == width:
            pass
        else:
            raise Exception('mask_width %d is larger than expected %d' % (mask_width, width))
    ...
    def get_blob_by_index(self, index):
        icon_blob, icon_name = self.get_icon_by_index(index)
        icon_image = icon_blob['image']
        icon_mask = icon_blob['mask']

        if self.is_random_background:
            # random choose background
            ...
            # pad icon_mask to fit icon and background image
            icon_mask = cv2.copyMakeBorder(icon_mask, pad_size, pad_size, pad_size, pad_size, cv2.BORDER_CONSTANT, value=0)
        ...
        icon_mask = self.pad_mask(icon_mask, height, width)
        icon_mask = ndarray_to_tensor(icon_mask, is_cuda=False)
        icon_mask = torch.unsqueeze(icon_mask, dim=0)

        this_blob = dict()
        this_blob['image'] = icon_image
        this_blob['mask'] = icon_mask
